...
class IndustrialSensemaker:
    ...
    def discover_narratives(self, df, n_clusters=5):
        ...
        for i in range(n_clusters):
            ...
            sentimientos = [self.sentiment_analyzer.predict(t).probas['POS'] - self.sentiment_analyzer.predict(t).probas['NEG'] for t in cluster_data['text'].tolist()[:20]]
...
if __name__ == "__main__":
    # Detectar si estamos en Linux (Parrot) o Windows
    ...
    if not df_raw.empty:
        ...
        for n in reporte_final:
            salud = "POSITIVA" if n['sentimiento_salud'] > 0.1 else "CRÍTICA" if n['sentimiento_salud'] < -0.1 else "NEUTRAL"
