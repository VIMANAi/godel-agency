...
print("Polars version:", pl.__version__)"""),
    ...
    ("code", """with open("../config/config.yaml") as f:
...
print("Catálogo de seguidores:", config['catalogo_seguidores'])"""),
    ...
    ("code", """posts_raw = pl.read_ndjson("../data/raw/facebook/fb_posts_tepic_2026-02-20.jsonl")
...
print("Comentarios crudos:", comments_raw.height)"""),
    ...
    ("code", """posts_silver = mapear_esquema_crudo(posts_raw, config["catalogo_seguidores"], "DIFNay")
...
print(comments_silver.head(3))"""),
    ...
    ("code", """Path("../data/silver/redes_sociales").mkdir(parents=True, exist_ok=True)
...
with open("../config/config.yaml") as f:
    ...
    ("code", """posts_silver = pl.read_parquet("../data/silver/redes_sociales/fb_posts_silver.parquet")
...
print("Registros limpios - Comentarios:", comments_clean.height)"""),
    ...
    ("code", """anomalias_posts = posts_clean.filter(pl.col("is_anomaly") == True)
...
if anomalias_posts.height > 0:
    ...
    ("code", """assert validar_esquema_silver(posts_clean)
...
print("GEMINI_API_KEY configurada:", "GEMINI_API_KEY" in os.environ)"""),
    ...
    ("code", """posts = pl.read_parquet("../data/silver/redes_sociales/fb_posts_clean.parquet")
...
print("Comentarios para análisis NLP:", comments.height)"""),
    ...
    ("code", """agente = AgenteSemanticoElectoral()
...
print("Clasificación NLP finalizada.")"""),
    ...
    ("code", """print("Resultados de Posts (Framing & Sentiment):")
...
print(comments_enriched.select(["candidato", "sentimiento", "framing", "keywords_extracted"]).head(5))"""),
    ...
    ("code", """os.makedirs("../data/gold/redes_sociales", exist_ok=True)
...
from src.graph.network_analyzer import build_entity_network, detect_louvain_communities, calculate_k_core, calcular_sincronia_cib"""),
    ...
    ("code", """posts_gold = pl.read_parquet("../data/gold/redes_sociales/fb_posts_gold.parquet")
comments_gold = pl.read_parquet("../data/gold/redes_sociales/fb_comments_gold.parquet")"""),
    ...
    ("code", """alertas_cib = calcular_sincronia_cib(posts_gold, delta_t_max=300.0)
...
for idx, alerta in enumerate(alertas_cib):
    ...
    ("code", """# Unificar datasets para extraer co-ocurrencia de keywords/entidades
...
G = build_entity_network(df_combined)"""),
    ...
    ("code", """partition = detect_louvain_communities(G)
...
for com_id, nodos in comunidades_dict.items():
    ...
    ("code", """G_core = calculate_k_core(G, k=2)
...
print(df_base.select(["candidato", "pdiv_score", "sentimiento_score", "volumen_score", "engagement_score"]))"""),
    ...
    ("code", """# Simularemos el escenario: ¿Qué pasa si Beatriz Estrada incrementa un 40%
