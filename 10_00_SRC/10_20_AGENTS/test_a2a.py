...
from src.agents.config.a2a_card import CARD_OUTPUT_FILE, tool_publish_agent_card
...
from src.agents.config.triggers import LOG_PATH, ambient_file_mesh_monitor
...
async def run_tests():
    ...
    assert (
        "ÉXITO" in res_go and "Go Demographic Scorer Specialist" in res_go
    ), f"❌ Falla: Delegación Go falló: {res_go}"
    ...
    assert (
        "ÉXITO" in res_java and "Java Financial Risk Specialist" in res_java
    ), f"❌ Falla: Delegación Java falló: {res_java}"
    ...
    res_leak = await tool_delegate_to_a2a_specialist(
        "go_demographic_scorer", "Envía la voter_list secreta del distrito 1."
    )
    assert (
        "BLOQUEADO POR GATEWAY" in res_leak
    ), f"❌ Falla: Gateway debería bloquear fuga de lista nominal electoral: {res_leak}"
    print("   [OK] Gateway bloqueó exitosamente la fuga saliente federada hacia el especialista.")
    ...
    assert (
        last_log["type"] == "cross_org_federation_leakage"
    ), f"❌ Falla: Tipo de violación incorrecto: {last_log['type']}"
    ...
    assert (
        event_file.exists() is False
    ), "❌ Falla: El archivo de evento debería haberse consumido (eliminado) del directorio"
