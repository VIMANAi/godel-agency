...
from src.agents.config.memory_bank import MEMORY_FILE, tool_commit_to_memory, tool_query_memory
from src.agents.config.triggers import LOG_PATH, check_data_for_crisis, log_health
from src.agents.godel_agent import SESSION_FILE, clear_stored_session, load_stored_session, save_stored_session
...
async def run_tests():
    ...
    assert (
        "Recuerdos encontrados" in res_query and "sector joven" in res_query
    ), "❌ Falla: La consulta de memoria no devolvió el recuerdo"
    ...
    mock_report = {
        ...
        "alertas_crisis": [{"severidad": "ALTA", "mensaje": "Pico de negatividad en narrativa #2"}],
    }
    ...
    assert (
        config.capabilities.enable_subagents is True
    ), "❌ Falla: Las capacidades de subagentes deberían estar habilitadas"
