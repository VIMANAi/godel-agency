...
try:
    ...
    try:
        ...
        from data_ingestion import AnonymizationConfig, DataIngestionPipeline
...
class PDIVPipeline:
    ...
    def setup_paths(self):
        """Configura rutas dinámicas o por variables de entorno"""
        ...
        if env_base_path:
            ...
            # Por defecto: raíz del proyecto (dos niveles arriba de src/core/)
    ...
    def load_processed_data(self, filename: str = None) -> pd.DataFrame:
        ...
        ingestion_pipeline = DataIngestionPipeline(config=config, base_path=self.base_path)
    ...
    def analyze_sentiment_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # --- FALLBACK A MOTOR LOCAL (LocalSentiment) ---
        print(f"--- Usando motor local {self.model_name} ---")
        try:
            from src.core.local_sentiment import LocalSentimentAnalyzer
        except ImportError:
            from core.local_sentiment import LocalSentimentAnalyzer

        analyzer = LocalSentimentAnalyzer(model_name=self.model_name)
        texts = df["text"].tolist()
        ...
        for i in range(0, len(texts), batch_size):
            ...
            for text in lote:
                resultado = analyzer.analyze(text)
                resultados.append(resultado)

        # Integrar sentimiento al DataFrame original
        df["sentimiento"] = [r.get("sentimiento", 0.0) for r in resultados]
        df["intensidad"] = [r.get("intensidad", 3) for r in resultados]
        df["tema"] = [r.get("tema", "general") for r in resultados]
        df["categoria"] = [r.get("categoria", "neutro") for r in resultados]
        df["es_bot"] = [r.get("es_bot", False) for r in resultados]
    ...
    def discover_narratives(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            return {"narrativas": narrativas, "metadata": analysis_meta}
            ...
            final_results = {
                ...
                "estadisticas_globales": {
                    ...
                    "registros_anonimizados": (
                        int(sentiment_df["anonymized"].sum()) if "anonymized" in sentiment_df.columns else 0
                    ),
                    "calidad_promedio": (
                        float(sentiment_df["quality_score"].mean()) if "quality_score" in sentiment_df.columns else 1.0
                    ),
                },
            }
...
"""
            for i, narrativa in enumerate(results["narrativas_descubiertas"]["narrativas"][:3]):
                salud = (
                    "POSITIVA"
                    if narrativa["sentimiento_salud"] > 0.1
                    else "CRÍTICA" if narrativa["sentimiento_salud"] < -0.1 else "NEUTRAL"
                )
