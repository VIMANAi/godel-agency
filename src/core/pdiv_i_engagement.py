...
class IEngagementModule:
    ...
    def calculate_engagement_score(self, df: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            lf = lf.with_columns((pl.col("likes") + pl.col("shares") + pl.col("comments")).alias("temp_interactions"))

            # 2. Ponderar por canal
            if "source" in df.columns:
                # Crear un mapeo usando un dataframe auxiliar de Polars
                source_df = pl.DataFrame(
                    {"source_lower": list(self.source_weights.keys()), "weight": list(self.source_weights.values())}
                ).lazy()
                ...
                lf = lf.with_columns(
                    (pl.col("temp_interactions") * pl.col("weight").fill_null(self.source_weights["default"])).alias(
                        "temp_weighted_interactions"
                    )
                )
            ...
            agg_df = (
                lf.group_by("candidato")
                .agg(pl.col("temp_weighted_interactions").sum().alias("total_engagement"))
                .collect()
            )

            # Convertir de vuelta a pandas para seguir con las interfaces existentes
            engagement_weighted = pd.Series(agg_df["total_engagement"].to_numpy(), index=agg_df["candidato"].to_numpy())
