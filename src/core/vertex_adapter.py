...
load_dotenv()
if not os.getenv("GEMINI_API_KEY") and not os.getenv("APIFY_TOKEN"):
    from pathlib import Path
    for p in [Path("/home/fratfn/vertex_env/.env"), Path(__file__).resolve().parents[3] / "vertex_env" / ".env", Path.home() / "vertex_env" / ".env"]:
        if p.exists():
            load_dotenv(p)
            break
    ...
    def __init__(self, project_id=PROJECT_ID, location=LOCATION):
        ...
        if not self.client:
            raise ValueError(
                ...
                "Por favor, revisa tus credenciales en el archivo .env\n"
            )
