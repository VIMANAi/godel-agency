        ...
        if self.historical_votes_path and Path(self.historical_votes_path).exists():
            try:
                ...
                if 'partido' in df.columns and 'votos_proporcion' in df.columns:
                    loaded = dict(zip(df['partido'].str.upper(), df['votos_proporcion']))
...
if __name__ == "__main__":
    ...
    pdiv_sim = {
        'Candidato A (Morena/Verde)': 75.5,
        'Candidato B (Alianza PAN/PRI)': 62.0,
        'Candidato C (MC)': 68.2
    }
    ...
    forecast = module.run_monte_carlo_simulation(
        pdiv_scores=pdiv_sim,
        candidate_party_map=candidate_party,
        undecided_ratio=0.15,
        volatility=0.08
    )
    ...
    for cand, metrics in forecast['candidatos'].items():
        ...
        print(f" -> Intervalo de Confianza (95%): [{metrics['intervalo_confianza_95'][0]:.2f}% - {metrics['intervalo_confianza_95'][1]:.2f}%]")
