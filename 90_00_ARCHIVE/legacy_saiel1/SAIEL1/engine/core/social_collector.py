...
class ApifyCollector:
        ...
        try:
            ...
            for item in self.client.dataset(run["defaultDatasetId"]).iterate_items():
                data.append({
                    "source": platform,
                    "target": target,
                    "candidato": target,  # Para compatibilidad con PDIV
    ...
    def _save_to_raw(self, data, filename):
        # Ruta dinámica compatible con Windows/Linux
        import os
        from pathlib import Path
        
        # Detectar sistema operativo
        if os.name == "nt":  # Windows
            base_path = Path("c:/Users/dcamb/Desktop/Consultoria_Inteligencia/data/raw")
        else:  # Linux/Parrot
            base_path = Path.home() / "Dev/Projects/SAIEL/data/raw"
